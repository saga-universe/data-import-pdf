...
use diesel::{insert_into, prelude::*, sql_query, FromSqlRow};
...
impl Status {
    ...
    pub fn find_status_in_array(list_of_status: &Vec<FullStatus>, value: &str) -> String {
        ...
        match status {
            Some(status) => String::from(status.id.to_string()),
            None => {
    ...
    pub fn create<'a>(conn: &mut MysqlConnection, records: [NewStatus<'a>; 5]) {
        ...
        let record_inserted = insert_into(status)
            .values::<Vec<NewStatus<'a>>>(records.to_vec())
            .execute(conn);
